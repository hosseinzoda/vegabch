...
export default class Moria1MintLoan extends VegaCommand<typeof Moria1MintLoan> {
  static args = {
  ...
  static examples = [
    `<%= config.bin %> <%= command.id %> <loan_amount> <collateral_amount> <annual_interest_rate>`,
  ];
  ...
  async run (): Promise<any> {
    ...
    const result = await this.callModuleMethod('moria1.mint-loan', wallet_name, { loan_amount, collateral_amount, annual_interest_bp }, { broadcast, txfee_per_byte, verify: true });
    this.log('Summary:');
    try {
