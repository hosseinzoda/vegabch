...
export default class Moria0RepayLoan extends VegaCommand<typeof Moria0RepayLoan> {
  ...
  async run (): Promise<any> {
    ...
    const result = await this.callModuleMethod('moria0.repay-loan', wallet_name, selected_loan, { broadcast, txfee_per_byte, verify: true });
    this.log('Summary:');
    try {
      const loan_params = MoriaV0.parseParametersFromLoanNFTCommitment(selected_loan.output.token.nft.commitment);
      this.log(`  Repay amount: ${bigIntToDecString(loan_params.amount, MUSDV0_DECIMALS)} ${MUSDV0_SYMBOL}`);
      this.log(`  Loan collateral: ${bigIntToDecString(selected_loan.output.amount, bch_token_info.decimals)} ${bch_token_info.symbol}`);
